...
export function MoreMenu({
  ...
  // Navigate to a dashboard route, exiting register mode
  function exitRegisterTo(href: string) {
    setActivePanel(null);
    setMode("dashboard");
    router.push(href);
  }
      ...
      return (
        <div className="p-3 space-y-1">
          ...
          {([
            ...
            { panel: "trade_in_nav" as const, icon: <span className="text-xl leading-none">{"\u21C4"}</span>, label: "Trade-In", action: () => exitRegisterTo("/dashboard/trade-ins/new") },
            ...
            ...(hasModule("cafe") ? [{ panel: "cafe_nav" as const, icon: <span className="text-xl leading-none">{"\u2615"}</span>, label: "Cafe", action: () => exitRegisterTo("/dashboard/cafe") }] : []),
          ...
          ] as Array<{ panel: ActivePanel | "trade_in_nav" | "cafe_nav"; icon: React.ReactNode; label: string; accent?: boolean; action?: () => void }>).map((item) => (
            <button
              key={item.panel}
              onClick={() => {
                if (item.action) {
                  ...
                  const next = item.panel as ActivePanel;
                  setActivePanel(next);
                  if (next === "void_last") loadLastVoidable();
                  if (next === "loyalty") loadLoyalty();
                  if (next === "order_lookup") loadOrderLookup();
                  if (next === "store_credit" && customer) {
