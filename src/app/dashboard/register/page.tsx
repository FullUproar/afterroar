...
export default function RegisterPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden select-none">
      ...
      {/* ====== PRINTABLE RECEIPT FOR ORDER LOOKUP (hidden on screen, shown on print) ====== */}
      {orderLookupReceipt && (
        <div className="print-receipt hidden">
          ...
          {orderLookupReceipt.staff_name && (
            <div className="receipt-customer">Cashier: {orderLookupReceipt.staff_name}</div>
          )}
          {orderLookupReceipt.customer_name && (
            <div className="receipt-customer">Customer: {orderLookupReceipt.customer_name}</div>
          )}
          <div className="receipt-divider">{"--------------------------------"}</div>
          ...
          <div className="receipt-line">
            <span>Subtotal</span>
            <span>{formatCents(orderLookupReceipt.subtotal_cents)}</span>
          </div>
          {orderLookupReceipt.discount_cents > 0 && (
            <div className="receipt-line">
              <span>Discount</span>
              <span>-{formatCents(orderLookupReceipt.discount_cents)}</span>
            </div>
          )}
          {orderLookupReceipt.tax_cents > 0 && (
            <div className="receipt-line">
              <span>Tax</span>
              <span>{formatCents(orderLookupReceipt.tax_cents)}</span>
            </div>
          )}
          {orderLookupReceipt.credit_applied_cents > 0 && (
            <div className="receipt-line">
              <span>Store Credit</span>
              <span>-{formatCents(orderLookupReceipt.credit_applied_cents)}</span>
            </div>
          )}
          {orderLookupReceipt.gift_card_applied_cents > 0 && (
            <div className="receipt-line">
              <span>Gift Card</span>
              <span>-{formatCents(orderLookupReceipt.gift_card_applied_cents)}</span>
            </div>
          )}
          {orderLookupReceipt.loyalty_discount_cents > 0 && (
            <div className="receipt-line">
              <span>Loyalty</span>
              <span>-{formatCents(orderLookupReceipt.loyalty_discount_cents)}</span>
            </div>
          )}
          <div className="receipt-divider">{"================================"}</div>
          <div className="receipt-line receipt-total">
            <span>TOTAL</span>
            <span>{formatCents(orderLookupReceipt.total_cents)}</span>
          </div>
          <div className="receipt-line">
            <span>Paid</span>
            <span>{orderLookupReceipt.payment_method.toUpperCase()}</span>
          </div>
          {orderLookupReceipt.change_cents > 0 && (
            <div className="receipt-line">
              <span>Change</span>
              <span>{formatCents(orderLookupReceipt.change_cents)}</span>
            </div>
          )}
          <div className="receipt-divider">{"--------------------------------"}</div>
          <div className="receipt-footer">
            {orderLookupReceipt.receipt_footer}
          </div>
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            <button
                              onClick={async () => {
                                ...
                                // Print will happen after receipt loads — triggered by the receipt view
