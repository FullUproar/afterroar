...
- [`seidr/`](./seidr/) — Old Norse magic and divination. **Phase 0 end-to-end with closed offline demo loop.** Profile-driven engine: 18-question quiz emits a 24-dim player vector, cosine-matched against game profiles, with natural-language explanations. Real quiz UI export JSON drops directly into `scripts/run-rec.mjs --player-profile` for demo. Includes deployable HTML quiz, 50 curated questions, 24 dimensions, schema (`rec_seidr_*`), LLM pipeline, profile validator, 7 hand-authored reference profiles, cosine matcher with subtle-wrongness suite, explanation generator, offline CLI runner with built-in archetypes, and quiz-UI-export interop. **169/169 seidr tests pass.** Awaiting only the top-500 LLM run (laptop, requires API key) for full corpus.
