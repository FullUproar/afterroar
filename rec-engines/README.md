...
- [`QUICKSTART.md`](./QUICKSTART.md) — 60-second "I want to see seidr work" guide.
...
- [`mimir/`](./mimir/) — Norse god of wisdom. **Foundation engine, mobile-buildable Phase 0 work complete.** Pure BGG metadata-based scoring. 182 tests green; migration runner + safety harness empirically validated against local Postgres (now detects INSERT/DELETE/UPDATE in addition to DDL). 3 migrations applied: 0001 (foundation schema), 0002 (4 dimension-framework node tables), 0003 (42 seed-taxonomy rows: Bartle + OCEAN + SDT + MDA + emotional palette + cognitive dims + named contexts).
...
- [`seidr/`](./seidr/) — Old Norse magic and divination. **Phase 0 end-to-end runnable against a 225-game corpus.** Profile-driven engine: 18-question quiz emits a 24-dim player vector, cosine-matched against game profiles, with natural-language explanations. Real quiz UI export JSON drops directly into `scripts/run-rec.mjs --game-profiles data/seed-game-profiles.json` for demo. Includes deployable HTML quiz, 50 curated questions, 24 dimensions, schema (`rec_seidr_*`), LLM pipeline, profile validator, 7 reference profiles + **225 hand-authored seed profiles spanning all 8 BGG subdomains**, cosine matcher with subtle-wrongness suite, explanation generator, offline CLI runner with built-in archetypes, and quiz-UI-export interop. **181/181 seidr tests pass.** Top-500 LLM-API run is optional for further calibration.
