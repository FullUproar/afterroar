...
// ============================================================================
// Seidr migration runner. Plain Node ES module (no build step).
//
// This runner reads migrations from `seidr/migrations/` and applies them.
// Per SILO.md § 8 (engines do not import from each other), this is a
// duplicate of mimir's runner (functionally identical) rather than a shared
// library. Each engine carries its own safety enforcement; only the
// rec_migrations bookkeeping table is shared across engines.
//
...
//      re-running is idempotent. Filenames must be unique across engines
//      (seidr's `0001_seidr_*` is distinct from mimir's `0001_create_rec_tables`).
//
// Usage:
//   DATABASE_URL=postgres://... node scripts/apply-migrations.mjs --dry-run
//   DATABASE_URL=postgres://... node scripts/apply-migrations.mjs
//
// Run AFTER mimir's runner -- seidr's tables don't depend on mimir's,
// but mimir's migrations are the platform's foundation schema.
//
// Tests live in seidr/tests/apply-migrations.test.mjs.
// ============================================================================
