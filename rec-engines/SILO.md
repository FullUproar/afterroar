...
| `seidr` | Phase 0 (full pipeline + matcher + explainer + CLI + quiz interop) | Sprints 1.0.18→1.0.21: schema, LLM pipeline, validator, 7 hand-authored reference profiles, cosine matcher with subtle-wrongness suite, explanation generator, offline CLI runner, and quiz-UI-export interop. **Offline demo loop is closed**: real quiz UI export JSON pipes directly into `run-rec.mjs --player-profile` to produce ranked recommendations with explanations. 169/169 tests pass. Top-500 LLM run (laptop, requires API key) is the only remaining gap. | Profile-driven; quiz → 24-dim vector → cosine match → ranked recommendations with natural-language explanations |
