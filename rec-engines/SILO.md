...
|---|---|---|---|
| `mimir` | Phase 0 | **End-to-end validated.** 182 tests green. Migrations 0001 + 0002 + 0003 sandbox-validated against local Postgres 16. 23 rec_* tables, 42 seed-taxonomy rows. Migration runner safety harness now detects INSERT/DELETE/UPDATE in addition to DDL. | Foundation engine; pure metadata-based scoring; the always-available cold-start baseline |
...
| `seidr` | Phase 0 (full pipeline + matcher + explainer + CLI + 225-game seed corpus) | Sprints 1.0.18→1.0.29: schema, LLM pipeline, validator, 7 hand-authored reference profiles, cosine matcher with subtle-wrongness suite, explanation generator, offline CLI runner, quiz-UI-export interop, and **225-game seed corpus** spanning all 8 BGG subdomains. **End-to-end runnable today**: real quiz UI export JSON pipes directly into `run-rec.mjs --game-profiles data/seed-game-profiles.json`. 283/283 tests pass. Top-500 LLM-API run is optional for further calibration. | Profile-driven; quiz → 24-dim vector → cosine match → ranked recommendations with natural-language explanations |
